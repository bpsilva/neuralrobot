...
//************************ aux_var **************************
  int aux_iNumeroOcultos;
  double aux_dPasso;
  unsigned long aux_ulRandomSeed;
  double bestEQM = 100.0;
...
void backup(double menorErro)
{
  aux_iNumeroOcultos = iNumeroOcultos;
  aux_dPasso = dPasso;
  aux_ulRandomSeed = ulRandomSeed;
  bestEQM = menorErro;
...
int main (int argc, char *argv[])
{
  ...
  cout<< "Melhor EQM: " << bestEQM<<endl;
  cout<< "-o "  << aux_iNumeroOcultos << " -p " << aux_dPasso <<" -i " <<  dInitPesos <<" -e " <<iMaximoEpocas << " -g " << iFreqGeneral <<" -r " << iFreqRelator << " -s " <<  aux_ulRandomSeed << endl;
    

  iNumeroOcultos = aux_iNumeroOcultos;
  dPasso = aux_dPasso;
  ulRandomSeed = aux_ulRandomSeed;
...
void GeraValores()
{
  ...
  iNumeroOcultos = (rand()%100) + 2;
  
  dInitPesos = INIT_PESOS;
  
  dPasso = 0.00001;
  
  iMaximoEpocas = MAX_EPOCAS;
  
  ulRandomSeed = rand();
  
  iFreqGeneral = FREQ_GENERAL;
  
  iFreqRelator = FREQ_RELATOR;

  iRealizarAprendizado  = 1;
  iEncerrarAprendizado = 0;   
